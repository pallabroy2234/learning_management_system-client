...
import {useCreateBannerMutation} from "../../../store/features/customization/customizationApi.ts";
...
const Hero = () => {
	const [uploadBanner, {isLoading:isBannerLoading,isSuccess:isBannerSuccess, isError:isBannerError, error:bannerError, data:bannerData}] = useCreateBannerMutation()
	...
	const imageWatch = watch("img") as unknown as FileList | undefined;
	...
	useEffect(() => {
		if(isBannerSuccess){
			const message = bannerData?.message || "Banner updated successfully";
				...
				<form onSubmit={handleSubmit(onSubmit)}>
                      ...
                      '
								...
								<motion.div variants={containerVariants} initial='hidden' animate='visible' className='space-y-4'>
									...
									<motion.p variants={itemVariants} className='500px:text-lg text-gray-600 dark:text-gray-300 text-base'>
										Join 500,000+ professionals in our immersive learning community. Transform your career with expert-led
										courses.
									</motion.p>
								</motion.div>
							)}
					...
					<div className='1100px:mt-[100px] mt-8 flex  1100px:justify-end'>
						<button
							...
							{isBannerLoading ? "Loading..." : <><FiSave className='mr-2' /> Save</>}
						</button>
					</div>
				</form>
			</div>
